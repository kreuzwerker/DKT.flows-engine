...
function StepFunctions() {
  ...
  function getWorkflowDefinition(workflow) {
    return readFile(fsUtil.getWorkflowPath(workflow)).then(res => JSON.parse(res)).catch((err) => {
  ...
  function deleteDeployedStateMachines(workflow) {
    return listDeployedStateMachines(workflow)
      .then((stateMachines) => {
        ...
        const deletePromises = stateMachines.map(sm =>
          stepFunctions
            .deleteStateMachine({
  ...
  function deployStateMachine(workflow) {
    const workflowName = workflow.indexOf('.json') > 0 ? workflow.split('.')[0] : workflow

    return getWorkflowDefinition(workflowName)
      .then(definition =>
        stepFunctions
          .createStateMachine({
            definition: JSON.stringify(definition),
            name: `${workflowName}-${uuid.v1()}`,
            roleArn: arn
          })
          .promise()
      )
  ...
  return {
    deploy: (workflow) => {
      return deleteDeployedStateMachines(workflow).then(() => deployStateMachine(workflow))
    },

    deployAll: () => {
      const workflows = fsUtil.getAllWorkflows()
      return deleteDeployedStateMachines().then(() =>
        Promise.all(workflows.map(flow => deployStateMachine(flow)))
      )
    },

    startExecution: (workflow, data) => {
      return listDeployedStateMachines(workflow).then((stateMachines) => {
