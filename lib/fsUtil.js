...
const { workflows, lambdas, services, dist } = settings.fs
...
function fsUtil() {
   ...
   * ---- services -------------------------------------------------------------
   */
  function getServiceTemplatePath(serviceName) {
    return Path.join(settings.fs.services.base, serviceName, 'template')
  }

  function getAllServices() {
    const serviceFns = Fs.readdirSync(services.base)
  ...
  function getLambdaPath(serviceName) {
    return Path.join(settings.fs.services.base, serviceName, 'lambda', lambdas.handler)
  }

  function lambdaExists(serviceName) {
    return Fs.existsSync(getLambdaPath(serviceName))
  }
  ...
  return {
    fromDir,
    getServiceTemplatePath, getAllServices,
