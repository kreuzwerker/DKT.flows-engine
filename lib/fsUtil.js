...
function fsUtil() {
  ...
  function getAllResourcesWithoutLambda() {
    const allResources = getAllResources()
    return allResources
      .filter(resource => !Fs.existsSync(Path.join(settings.fs.resources.base, resource, 'lambda')))
  ...
  function getServiceResources() {
    const services = Fs.readdirSync(resources.services.base).filter(
      file => file.indexOf('.') === -1
    )
    const serviceDefs = services
      .map((service) => {
        const serviceDef = Path.join(resources.services.base, service, 'service')
        if (Fs.existsSync(`${serviceDef}.js`) && service !== 'task') {
  ...
  return {
    fromDir,
    getResourceTemplatePath,
    getResourceSwaggerPath,
    getAllResources,
    getAllResourcesWithLambda,
    getAllResourcesWithoutLambda,
    getLambdaPath,
    getServiceResources,
    getAllWorkflows,
    getWorkflowPath,
